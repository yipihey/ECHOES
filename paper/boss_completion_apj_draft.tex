...
weights.  The released posterior is compact: one 3.14 MB file stores the fixed
...
3.14 MB: the fixed observed catalog, the base positions and provenance for the
...
Appendix~\ref{app:products}).  A matching
...
prior does not degrade the failures relative to collisions.  The per-object
ensemble is mildly over-confident for both kinds (the rank histogram of the true
redshift departs from uniform at the $\sim0.1$ Kolmogorov--Smirnov level), a
small residual that is conservative for ensemble-mean statistics and bounded by
the statistic-level calibration below.
...
quantile levels and quantized to \texttt{uint16}, plus the field-correlation copula modes, for a 3.14 MB total), the
