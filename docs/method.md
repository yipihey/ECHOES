...
The missing redshifts are not drawn independently: a Gaussian **copula** built from the
measured ξ(r) gives them the coherent cross-object dependence of the density field, so
the *joint* completion covariance is honest (not ~19% under-dispersed) while each
per-object marginal — hence the PIT calibration of §8 — is unchanged. `copula=False`
