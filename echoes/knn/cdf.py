...
clustering observables. See ``echoes.knn.derived`` for the
reductions.
...
an optional Rust cascade backend (not ported here). ``backend
...
def joint_knn_cdf(
    ...
    if backend in ("cascade", "rust"):
        # Dispatch to the optional Rust cascade backend (not ported to ECHOES). The CLI binding
